...
<!-- Converts xLMNL format into a structured form.
    
     Modified from xlmnl-xml.xsl
     
     Accepts an $elements parameter carrying a sequence 
     of strings naming range types to be represented as enclosures.
     
     The stylesheet also provides functions accepting
...
<!--
  ...
  <xsl:template match="x:document">
    ...
    <xsl:copy>
      ...
      <xsl:attribute name="start" select="(x:content/x:span[1]/@start,0)[1]"/>
      <xsl:attribute name="end" select="(x:content/x:span[last()]/@end,0)[1]"/>
      <!--<xsl:call-template name="annotation-attributes"/>-->
      <xsl:apply-templates select="x:annotation | x:content"/>
    </xsl:copy>    
  </xsl:template>
  ...
  <xsl:template match="x:annotation">
    <xsl:copy>
      ...
      <xsl:attribute name="aID" select="@ID"/>
      <xsl:apply-templates>
    ...
    <xsl:for-each-group select="$spans" group-adjacent="(tokenize(@elements,'\s+')[$level],'')[1]">
      ...
      <xsl:choose>
        <xsl:when test="exists($range)">
          <xsl:call-template name="make-enclosure">
  ...
  <xsl:template name="make-enclosure">
    <xsl:param name="range" required="yes"/>
    <xsl:param name="contents" select="()"/>
    <x:div rID="{$range/@ID}">
      <xsl:copy-of select="$range/(@name|@start|@end)"/>
      <xsl:apply-templates select="$range/x:annotation"/>
    ...
    <!--<xsl:element name="{($range/@name[f:okay-name(.)],'e')[1]}">
      ...
      <xsl:for-each select="$range">
        <xsl:apply-templates select="x:annotation"/>
        <!-\-<xsl:call-template name="annotation-attributes"/>-\->
      </xsl:for-each>
      <xsl:copy-of select="$contents"/>
    </xsl:element>-->
  ...
  <xsl:template match="*" mode="tag">
    <xsl:copy>
      <xsl:copy-of select="(: @* except @elements :) (@name|@start|@end)"/>
  ...
  <xsl:template match="x:span" mode="tag">
    ...
    <xsl:copy>
      <xsl:copy-of select="@ranges | @start | @end"/>
      <xsl:apply-templates mode="tag"/>
    </xsl:copy>
  ...
  <xsl:template match="x:range" mode="start-tag">
    <x:start rID="{@ID}">
      <xsl:copy-of select="(: @* except @ID :) (@name|@start|@end)"/>
      <xsl:apply-templates/>
    </x:start>
  </xsl:template>
  
  <xsl:template name="make-empty">
    <xsl:param name="range" required="yes"/>
    <xsl:param name="contents" select="()"/>
  </xsl:template>
  ...
  <xsl:template match="x:range[@start eq @end]" mode="end-tag"/>
