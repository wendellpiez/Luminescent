  ...
  <xsl:variable name="ranges">
    <xsl:choose>
      <xsl:when test="$doctype='tei'">
        <!--<arcs color="gold" stroke="gold">
          <ranges>TEI.2 teiHeader fileDesc titleStmt title editionStmt edition
            publicationStmt availability date sourceDesc encodingDesc
            editorialDecl text body</ranges>
        </arcs>-->
        <arcs color="darkred">
          <ranges>p</ranges>
        </arcs>
      ...
      <xsl:otherwise>
        ...
        <!--        <xsl:for-each-group select="$lmnl-document/x:range/@name" group-by=".">
          <arcs fill="blue" fill-opacity="0.2" stroke="midnightblue" stroke-width="0.5">
            <ranges>
